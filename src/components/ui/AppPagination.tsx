...
export const AppPagination: React.FC<Props> = ({ current, total, onChange, maxVisible = 5 }) => {
  ...
  const btnBase =
    "flex h-8 w-8 items-center justify-center rounded-lg border text-[13px] font-bold transition-all active:scale-95";
  const btnActive = "bg-primary border-primary text-white shadow-[var(--shadow-md)] shadow-primary/20";
  const btnInactive =
    "bg-card border-[var(--border)] text-secondary-foreground hover:border-primary hover:text-primary hover:bg-primary-light-bg/30";
