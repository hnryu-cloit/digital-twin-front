...
import {
  ...
  Music, SlidersHorizontal, ChevronLeft, ChevronRight, Filter, RotateCcw,
} from "lucide-react";
...
const SEGMENT_COLORS: Record<Segment, { bg: string; text: string; border: string }> = {
  "Gamer":         { bg: "#EEF1FF", text: "#3D5AF1", border: "#C7D2FE" },
...
const RAW_PERSONAS: Omit<Persona, "id">[] = [
  { name:"이준혁", age:28, gender:"남성", occupation:"게임 개발자",     device:"Galaxy S24 Ultra", segments:["Gamer","Early Adopter"],   keywords:["카메라","성능","배터리"],        purchaseIntent:92, iconKey:0, color:"#3D5AF1", iconBg:"#EEF1FF", description:"최신 모바일 기술에 관심 많은 20대 남성 게이머. 고성��� 디바이스 선호." },
  ...
  { name:"윤성호", age:19, gender:"남성", occupation:"고등학생",        device:"Galaxy A54",       segments:["Gamer","Trendsetter"],         keywords:["게임","카메라","SNS"],          purchaseIntent:48, iconKey:0, color:"#3D5AF1", iconBg:"#EEF1FF", description:"게임과 SNS를 즐기는 10대 학생." },
  ...
  { name:"송민기", age:30, gender:"남성", occupation:"프리랜서 개발자", device:"Galaxy S24 Ultra", segments:["Gamer","Early Adopter"],       keywords:["성능","AI 기능","줌 기능"],   purchaseIntent:80, iconKey:0, color:"#3D5AF1", iconBg:"#EEF1FF", description:"최신 기술에 관심 많은 30대 프리랜서." },
  ...
  { name:"남상우", age:18, gender:"남성", occupation:"고등학생",        device:"Galaxy A54",       segments:["Gamer","Trendsetter"],          keywords:["게임","성능","SNS"],           purchaseIntent:43, iconKey:0, color:"#3D5AF1", iconBg:"#EEF1FF", description:"게임과 SNS가 주요 스마트폰 활용 목적인 10대." },
  ...
  { name:"장준혁", age:32, gender:"남성", occupation:"게임 스트리머",   device:"Galaxy S24 Ultra", segments:["Gamer","Premium Buyer"],        keywords:["성능","배터리","스트리밍"],     purchaseIntent:96, iconKey:0, color:"#3D5AF1", iconBg:"#EEF1FF", description:"고성능 기기가 필수인 전업 게임 스트리머." },
...
function IntentBar({ value, color }: { value: number; color: string }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <span style={{ fontSize: 12, fontWeight: 700, color: "#1E293B", minWidth: 30, textAlign: "right" }}>{value}%</span>
    </div>
  );
}
...
function PersonaCard({ persona, onEdit, onDelete }: { persona: Persona; onEdit: (p: Persona) => void; onDelete: (id: string) => void }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#E8ECF4] shadow-sm p-5 flex flex-col gap-3 relative group hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p style={{ fontSize: 14, fontWeight: 700, color: "#1E293B" }}>{persona.name}</p>
            <p style={{ fontSize: 12, color: "#64748B" }}>{persona.age}세 · {persona.gender} · {persona.occupation}</p>
          </div>
        </div>
        <div className="relative" ref={menuRef}>
          <button onClick={() => setMenuOpen((p) => !p)}
            className="w-7 h-7 rounded-lg bg-[#F4F6FB] hover:bg-[#EEF1FF] flex items-center justify-center transition-colors opacity-0 group-hover:opacity-100">
            <MoreHorizontal size={14} className="text-[#64748B]" />
          </button>
          {menuOpen && (
            <div className="absolute right-0 top-8 z-30 bg-white border border-[#E8ECF4] rounded-xl shadow-lg py-1.5 w-32">
              <button onClick={() => { onEdit(persona); setMenuOpen(false); }}
                className="w-full flex items-center gap-2 px-3 py-1.5 hover:bg-[#F1F5FF] transition-colors">
                <Pencil size={12} className="text-[#3D5AF1]" />
                <span style={{ fontSize: 12, color: "#1E293B" }}>편집</span>
              </button>
      ...
      <p className="line-clamp-2" style={{ fontSize: 12, color: "#64748B", lineHeight: 1.6 }}>{persona.description}</p>

      <div className="flex items-center gap-1.5">
        <Smartphone size={12} className="text-[#94A3B8]" />
        <span style={{ fontSize: 12, color: "#475569" }}>{persona.device}</span>
      </div>
      ...
      <div className="flex flex-wrap gap-1">
        {persona.keywords.slice(0, 4).map((kw) => (
          <span key={kw} className="bg-[#F8FAFC] border border-[#E2E8F0] text-[#64748B] px-2 py-0.5 rounded-full"
            style={{ fontSize: 10 }}>
            # {kw}
          </span>
        ))}
      ...
      <div>
        <span style={{ fontSize: 11, color: "#94A3B8" }}>구매 의향</span>
      ...
      <button
        ...
        className="absolute bottom-4 right-4 flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-[#EEF1FF] text-[#3D5AF1] border border-[#C7D2FE] opacity-0 group-hover:opacity-100 transition-opacity hover:bg-[#3D5AF1] hover:text-white"
...
const EMPTY: Omit<Persona, "id"> = {
  ...
  color: "#3D5AF1", iconBg: "#EEF1FF", iconKey: 0, description: "",
};
...
function EditModal({ initial, onSave, onClose }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-xl border border-[#E8ECF4] w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#E8ECF4]">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-lg bg-[#3D5AF1] flex items-center justify-center">
          ...
          <button onClick={onClose} className="w-8 h-8 rounded-lg bg-[#F4F6FB] hover:bg-[#EEF1FF] flex items-center justify-center transition-colors">
        ...
        <div className="overflow-y-auto px-6 py-5 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block mb-1.5" style={{ fontSize: 12, fontWeight: 700, color: "#475569" }}>이름</label>
              <input className="w-full border border-[#E8ECF4] rounded-lg px-3 py-2 outline-none focus:border-[#3D5AF1] bg-[#F8FAFC] focus:bg-white transition-colors"
                style={{ fontSize: 13, color: "#1E293B" }} placeholder="홍길동"
                value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} />
            </div>
            <div>
              <label className="block mb-1.5" style={{ fontSize: 12, fontWeight: 700, color: "#475569" }}>나이</label>
              <input type="number" className="w-full border border-[#E8ECF4] rounded-lg px-3 py-2 outline-none focus:border-[#3D5AF1] bg-[#F8FAFC] focus:bg-white transition-colors"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block mb-1.5" style={{ fontSize: 12, fontWeight: 700, color: "#475569" }}>성별</label>
                ...
                {(["남성", "여성"] as Gender[]).map((g) => (
                  <button key={g} onClick={() => setForm((p) => ({ ...p, gender: g }))}
                    ...
                    style={{ fontSize: 13, fontWeight: form.gender === g ? 700 : 500,
                      backgroundColor: form.gender === g ? "#EEF1FF" : "#F8FAFC",
                      borderColor: form.gender === g ? "#3D5AF1" : "#E8ECF4",
                      color: form.gender === g ? "#3D5AF1" : "#475569" }}>
                    {g}
                  </button>
                ))}
              </div>
            ...
            <div>
              <label className="block mb-1.5" style={{ fontSize: 12, fontWeight: 700, color: "#475569" }}>직업</label>
              <input className="w-full border border-[#E8ECF4] rounded-lg px-3 py-2 outline-none focus:border-[#3D5AF1] bg-[#F8FAFC] focus:bg-white transition-colors"
                style={{ fontSize: 13, color: "#1E293B" }} placeholder="직업을 입력하세요"
          ...
          {/* Device */}
          <div>
            <label className="block mb-1.5" style={{ fontSize: 12, fontWeight: 700, color: "#475569" }}>사용 기기</label>
            <div className="relative">
              <select className="w-full border border-[#E8ECF4] rounded-lg px-3 py-2 outline-none focus:border-[#3D5AF1] bg-[#F8FAFC] appearance-none"
                style={{ fontSize: 13, color: "#1E293B" }}
          ...
          {/* Segments */}
          <div>
            <label className="block mb-1.5" style={{ fontSize: 12, fontWeight: 700, color: "#475569" }}>세그먼트</label>
            <div className="border border-[#E8ECF4] rounded-lg px-3 py-2 bg-[#F8FAFC] flex flex-wrap gap-1.5 cursor-pointer min-h-[40px]"
              onClick={() => setSegOpen((p) => !p)}>
              {form.segments.length === 0 && <span style={{ fontSize: 13, color: "#CBD5E1" }}>세그먼트 선택...</span>}
            ...
            {segOpen && (
              <div className="border border-[#E8ECF4] rounded-lg mt-1 bg-white shadow-lg overflow-hidden z-10 relative">
                {ALL_SEGMENTS.map((seg) => {
                  const c = SEGMENT_COLORS[seg];
                  const active = form.segments.includes(seg);
                  return (
                    <button key={seg} onClick={() => toggleSeg(seg)}
                      className="w-full flex items-center justify-between px-3 py-2 hover:bg-[#F4F6FB] transition-colors">
                      <span className="px-2 py-0.5 rounded-full border"
                        style={{ fontSize: 11, fontWeight: 600, backgroundColor: c.bg, color: c.text, borderColor: c.border }}>{seg}</span>
                      {active && <Check size={13} className="text-[#3D5AF1]" />}
                    </button>
                  );
                })}
              </div>
          ...
          {/* Keywords */}
          <div>
            <label className="block mb-1.5" style={{ fontSize: 12, fontWeight: 700, color: "#475569" }}>관심 키워드</label>
            <div className="flex gap-2 mb-2">
              <input className="flex-1 border border-[#E8ECF4] rounded-lg px-3 py-2 outline-none focus:border-[#3D5AF1] bg-[#F8FAFC] focus:bg-white transition-colors"
                style={{ fontSize: 13, color: "#1E293B" }} placeholder="키워드 입력 후 Enter"
                value={kwInput} onChange={(e) => setKwInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addKw()} />
              <button onClick={addKw} className="px-3 py-2 bg-[#3D5AF1] text-white rounded-lg hover:bg-[#2B46D9] transition-colors"
                style={{ fontSize: 12 }}>추가</button>
            </div>
            <div className="flex flex-wrap gap-1.5">
              {form.keywords.map((kw) => (
                <span key={kw} className="flex items-center gap-1 bg-[#F8FAFC] border border-[#E2E8F0] text-[#475569] px-2 py-0.5 rounded-full"
                  style={{ fontSize: 11 }}>
                  # {kw}
                  <button onClick={() => setForm((p) => ({ ...p, keywords: p.keywords.filter((k) => k !== kw) }))}
                    className="text-[#CBD5E1] hover:text-[#EF4444] transition-colors"><X size={10} /></button>
                </span>
              ))}
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label style={{ fontSize: 12, fontWeight: 700, color: "#475569" }}>구매 의향</label>
              <span className="bg-[#EEF1FF] text-[#3D5AF1] px-2 py-0.5 rounded-full" style={{ fontSize: 11, fontWeight: 700 }}>
                {form.purchaseIntent}%
              </span>
            </div>
            <input type="range" min={0} max={100} value={form.purchaseIntent}
              onChange={(e) => setForm((p) => ({ ...p, purchaseIntent: Number(e.target.value) }))}
              className="w-full accent-[#3D5AF1]" />
            <div className="flex justify-between"><span style={{ fontSize: 10, color: "#CBD5E1" }}>0%</span><span style={{ fontSize: 10, color: "#CBD5E1" }}>100%</span></div>
          </div>

          {/* Description */}
          <div>
            <label className="block mb-1.5" style={{ fontSize: 12, fontWeight: 700, color: "#475569" }}>설명</label>
            <textarea className="w-full border border-[#E8ECF4] rounded-lg px-3 py-2 outline-none focus:border-[#3D5AF1] bg-[#F8FAFC] focus:bg-white transition-colors resize-none"
              style={{ fontSize: 13, color: "#1E293B", lineHeight: 1.6 }} rows={3}
              placeholder="페르소나에 대한 설명을 입력하세요..."
              value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} />
          </div>
        </div>

        <div className="flex gap-2 px-6 py-4 border-t border-[#E8ECF4]">
          <button onClick={onClose} className="flex-1 py-2.5 border border-[#E8ECF4] rounded-lg text-[#475569] hover:bg-[#F4F6FB] transition-colors"
            style={{ fontSize: 13, fontWeight: 600 }}>취소</button>
          <button onClick={() => onSave(form)} disabled={!form.name.trim() || !form.occupation.trim()}
            className="flex-1 py-2.5 bg-[#3D5AF1] text-white rounded-lg hover:bg-[#2B46D9] transition-colors disabled:opacity-40 disabled:cursor-not-allowed shadow-sm"
...
function AdvancedFilterPopup({ filter, onChange, onClose, onReset }: {
  ...
  return (
    <div ref={ref} className="absolute top-full left-0 mt-2 z-40 bg-white rounded-2xl border border-[#E8ECF4] shadow-2xl w-[520px] overflow-hidden"
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-[#F1F5F9]">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-lg bg-[#EEF1FF] flex items-center justify-center">
            <SlidersHorizontal size={14} className="text-[#3D5AF1]" />
          </div>
          <span style={{ fontSize: 14, fontWeight: 800, color: "#1E293B" }}>상세 필터</span>
          {activeCount > 0 && (
            <span className="w-5 h-5 bg-[#3D5AF1] text-white rounded-full flex items-center justify-center"
              style={{ fontSize: 10, fontWeight: 700 }}>{activeCount}</span>
          )}
        </div>
        <button onClick={onClose} className="w-7 h-7 rounded-lg bg-[#F4F6FB] hover:bg-[#EEF1FF] flex items-center justify-center transition-colors">
      ...
      <div className="px-5 py-4 flex flex-col gap-5 max-h-[520px] overflow-y-auto">
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            <div className="w-1 h-4 bg-[#3D5AF1] rounded-full" />
            <span style={{ fontSize: 12, fontWeight: 800, color: "#1E293B" }}>인구통계</span>
          </div>
          <div className="grid grid-cols-2 gap-4">
            {/* Gender */}
            <div>
              <p style={{ fontSize: 11, color: "#94A3B8", fontWeight: 600, marginBottom: 8 }}>성별</p>
              <div className="flex gap-2">
                {(["남성", "여성"] as Gender[]).map((g) => (
                  <button key={g} onClick={() => setLocal((p) => ({ ...p, genders: toggle(p.genders, g) }))}
                    ...
                    style={{
                      ...
                      backgroundColor: local.genders.includes(g) ? "#EEF1FF" : "#F8FAFC",
                      borderColor: local.genders.includes(g) ? "#3D5AF1" : "#E8ECF4",
                      color: local.genders.includes(g) ? "#3D5AF1" : "#64748B",
                    }}>
                    {g}
                  </button>
                ))}
            ...
            {/* Age */}
            <div>
              <p style={{ fontSize: 11, color: "#94A3B8", fontWeight: 600, marginBottom: 8 }}>연령대</p>
              <div className="flex flex-wrap gap-1.5">
                {AGE_GROUPS.map((ag) => (
                  <button key={ag} onClick={() => setLocal((p) => ({ ...p, ageGroups: toggle(p.ageGroups, ag) }))}
                    ...
                    style={{
                      ...
                      backgroundColor: local.ageGroups.includes(ag) ? "#EEF1FF" : "#F8FAFC",
                      borderColor: local.ageGroups.includes(ag) ? "#3D5AF1" : "#E8ECF4",
                      color: local.ageGroups.includes(ag) ? "#3D5AF1" : "#64748B",
                    }}>
                    {ag}
                  </button>
                ))}
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <span style={{ fontSize: 12, fontWeight: 800, color: "#1E293B" }}>사용 기기</span>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {DEVICES.map((d) => (
              <button key={d} onClick={() => setLocal((p) => ({ ...p, devices: toggle(p.devices, d) }))}
                ...
                style={{
                  ...
                  backgroundColor: local.devices.includes(d) ? "#F5F3FF" : "#F8FAFC",
                  borderColor: local.devices.includes(d) ? "#8B5CF6" : "#E8ECF4",
                  color: local.devices.includes(d) ? "#7C3AED" : "#64748B",
                }}>
                <Smartphone size={11} />
                {d}
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <span style={{ fontSize: 12, fontWeight: 800, color: "#1E293B" }}>세그먼트</span>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {ALL_SEGMENTS.map((seg) => {
              const c = SEGMENT_COLORS[seg];
              ...
              return (
                <button key={seg} onClick={() => setLocal((p) => ({ ...p, segments: toggle(p.segments, seg) }))}
                  ...
                  style={{ fontSize: 11, fontWeight: active ? 700 : 500,
                    ...
                    borderColor: active ? c.border : "#E8ECF4",
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <span style={{ fontSize: 12, fontWeight: 800, color: "#1E293B" }}>구매 의향 범위</span>
          </div>
          <div className="flex items-center gap-3">
            <div className="flex-1">
              <div className="flex items-center justify-between mb-1">
                <span style={{ fontSize: 11, color: "#94A3B8" }}>최소</span>
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between mb-1">
                <span style={{ fontSize: 11, color: "#94A3B8" }}>최대</span>
                <span className="bg-[#FFF7ED] text-[#F59E0B] px-2 py-0.5 rounded-full border border-[#FDE68A]"
      ...
      <div className="flex gap-2 px-5 py-4 border-t border-[#F1F5F9]">
        <button onClick={reset} className="flex items-center gap-1.5 px-4 py-2.5 border border-[#E8ECF4] rounded-xl text-[#94A3B8] hover:bg-[#F4F6FB] transition-colors"
          style={{ fontSize: 12, fontWeight: 600 }}>
          <RotateCcw size={12} />초기화
        </button>
        <button onClick={apply} className="flex-1 py-2.5 bg-[#3D5AF1] text-white rounded-xl hover:bg-[#2B46D9] transition-colors shadow-md"
          style={{ fontSize: 13, fontWeight: 700, boxShadow: "0 4px 12px #3D5AF130" }}>
          필터 적용
        </button>
      </div>
    </div>
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <button
        ...
        disabled={current === 1}
        className="flex items-center gap-1 px-3 py-1.5 rounded-full transition-all disabled:opacity-25 disabled:cursor-not-allowed hover:bg-[#EEF1FF]"
        style={{ fontSize: 12, fontWeight: 600, color: "#64748B", border: "1.5px solid #E8ECF4", backgroundColor: "#F8FAFC" }}
      >
        <ChevronLeft size={13} />
        이전
      </button>
      ...
      <div
        ...
        style={{ backgroundColor: "#F1F5F9", border: "1.5px solid #E2E8F0", padding: "4px 6px" }}
      >
          ...
          return (
            <button
              ...
              style={{
                ...
                backgroundColor: isActive ? "#3D5AF1" : "transparent",
                color: isActive ? "#fff" : isDisabled ? "#CBD5E1" : "#64748B",
                cursor: isDisabled ? "default" : "pointer",
                transition: "background-color 0.15s, color 0.15s, box-shadow 0.15s",
                boxShadow: isActive ? "0 2px 10px #3D5AF14D" : "none",
...
export const PersonaManagerPage: React.FC = () => {
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#F4F6FB] overflow-hidden">
      ...
      {/* Header */}
      <div className="bg-white border-b border-[#E8ECF4] px-6 pt-5 pb-4 flex items-start justify-between gap-4">
        <div>
          <p style={{ fontSize: 11, color: "#3D5AF1", fontWeight: 600, letterSpacing: "0.06em" }}>PERSONA</p>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: "#1E293B", lineHeight: 1.3 }}>페르소나 관리</h1>
          <p style={{ fontSize: 13, color: "#64748B", marginTop: 4 }}>시스템에 등록된 사용자 페르소나를 관리합니다.</p>
        </div>
        <button onClick={() => { setEditTarget(undefined); setModalOpen(true); }}
          className="flex items-center gap-2 bg-[#3D5AF1] text-white px-4 py-2.5 rounded-xl hover:bg-[#2B46D9] transition-colors shadow-md shrink-0"
          style={{ fontSize: 13, fontWeight: 600, boxShadow: "0 4px 12px #3D5AF130" }}>
          <Plus size={15} />페르소나 추가
        </button>
      </div>

      {/* Toolbar */}
      <div className="bg-white border-b border-[#E8ECF4] px-6 py-3 flex items-center gap-3">
        {/* Search */}
        <div className="flex items-center gap-2 bg-[#F4F6FB] border border-[#E8ECF4] rounded-xl px-3 py-2 flex-1 max-w-80">
          <Search size={14} className="text-[#94A3B8] shrink-0" />
          <input className="bg-transparent outline-none flex-1 text-[#1E293B] placeholder:text-[#CBD5E1]"
            style={{ fontSize: 13 }} placeholder="이름, 직업, 키워드 검색..."
            value={search} onChange={(e) => handleSearch(e.target.value)} />
          {search && (
            <button onClick={() => handleSearch("")}><X size={13} className="text-[#CBD5E1] hover:text-[#94A3B8]" /></button>
          )}
        </div>
          ...
          <button onClick={() => setFilterOpen((p) => !p)}
            className={`flex items-center gap-2 px-3 py-2 rounded-xl border transition-colors ${
              filterOpen || activeFilterCount > 0
                ? "bg-[#EEF1FF] border-[#3D5AF1] text-[#3D5AF1]"
                : "bg-[#F4F6FB] border-[#E8ECF4] text-[#64748B] hover:bg-[#EEF1FF] hover:border-[#C7D2FE]"
            }`}
            style={{ fontSize: 12, fontWeight: 600 }}>
        ...
        {advFilter.genders.map((g) => (
          <span key={g} className="flex items-center gap-1 bg-[#EEF1FF] text-[#3D5AF1] border border-[#C7D2FE] px-2.5 py-1 rounded-full"
            style={{ fontSize: 11, fontWeight: 600 }}>
            {g}
        ...
        {advFilter.ageGroups.map((ag) => (
          <span key={ag} className="flex items-center gap-1 bg-[#EEF1FF] text-[#3D5AF1] border border-[#C7D2FE] px-2.5 py-1 rounded-full"
            style={{ fontSize: 11, fontWeight: 600 }}>
            {ag}
        ...
        <div className="ml-auto flex items-center gap-1.5 shrink-0">
          ...
          <span style={{ fontSize: 12, color: "#64748B" }}>
            <span style={{ fontWeight: 700, color: "#3D5AF1" }}>{filtered.length}</span> / {personas.length}명
          </span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 pt-5 pb-2">
        {paginated.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-64 text-center">
            <div className="w-14 h-14 rounded-2xl bg-[#EEF1FF] flex items-center justify-center mb-3">
              <Users size={24} className="text-[#3D5AF1]" />
            </div>
            <p style={{ fontSize: 15, fontWeight: 600, color: "#1E293B" }}>검색 결과가 없습니다</p>
            <p style={{ fontSize: 13, color: "#94A3B8", marginTop: 4 }}>필터 조건을 변경해보세요.</p>
            {activeFilterCount > 0 && (
              <button onClick={() => setAdvFilter({ ...DEFAULT_FILTER })}
                className="mt-3 flex items-center gap-1.5 px-4 py-2 rounded-xl bg-[#3D5AF1] text-white hover:bg-[#2B46D9] transition-colors"
                style={{ fontSize: 12, fontWeight: 600 }}>
      ...
      {/* Pagination Bar */}
      <div className="bg-white border-t border-[#E8ECF4] px-6 py-3 flex items-center justify-between shrink-0">
        <p style={{ fontSize: 12, color: "#94A3B8" }}>
          <span style={{ fontWeight: 700, color: "#1E293B" }}>{(safePage - 1) * PAGE_SIZE + 1}</span>
          {" "}–{" "}
          <span style={{ fontWeight: 700, color: "#1E293B" }}>{Math.min(safePage * PAGE_SIZE, filtered.length) || 0}</span>
          {" "}/ 총{" "}
          <span style={{ fontWeight: 700, color: "#3D5AF1" }}>{filtered.length}</span>명
        </p>
        <Pagination current={safePage} total={totalPages} onChange={(p) => { setPage(p); }} />
        <div className="flex items-center gap-2">
          <span style={{ fontSize: 12, color: "#94A3B8" }}>페이지당</span>
          <span className="bg-[#EEF1FF] text-[#3D5AF1] px-2.5 py-1 rounded-lg border border-[#C7D2FE]"
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl border border-[#E8ECF4] p-6 w-80">
            <div className="w-12 h-12 rounded-xl bg-[#FFF0F0] flex items-center justify-center mb-4">
              <Trash2 size={20} className="text-[#EF4444]" />
            </div>
            <h3 style={{ fontSize: 16, fontWeight: 700, color: "#1E293B", marginBottom: 6 }}>페르소나 삭제</h3>
            <p style={{ fontSize: 13, color: "#64748B", lineHeight: 1.6, marginBottom: 20 }}>
              이 페르소나를 삭제하면 복구할 수 없습니다. 계속하시겠습니까?
            </p>
            <div className="flex gap-2">
              <button onClick={() => setDeleteConfirm(null)}
                className="flex-1 py-2.5 border border-[#E8ECF4] rounded-lg text-[#475569] hover:bg-[#F4F6FB] transition-colors"
                style={{ fontSize: 13, fontWeight: 600 }}>취소</button>
