...
const SEGMENT_COLORS: Record<Segment, { bg: string; text: string; border: string }> = {
  "MZ 얼리어답터": { bg: "var(--tag-pink-bg)", text: "var(--tag-pink)", border: "var(--tag-pink-bg)" },
  "프리미엄 구매자": { bg: "var(--tag-amber-bg)", text: "var(--tag-amber)", border: "var(--tag-amber-bg)" },
  "실용 중시 가족형": { bg: "var(--tag-green-bg)", text: "var(--tag-green)", border: "var(--tag-green-bg)" },
  "게이밍 성향군": { bg: "var(--tag-purple-bg)", text: "var(--tag-purple)", border: "var(--tag-purple-bg)" },
  "비즈니스 프로": { bg: "var(--tag-teal-bg)", text: "var(--tag-teal)", border: "var(--tag-teal-bg)" },
};
...
const RAW_PERSONAS: Omit<Persona,"id">[] = [
 ...
 { name:"강현우", age:19, gender:"남성", occupation:"고등학생", device:"S23 FE", segments:["게이밍 성향군"], keywords:["게임","디스플레이","가성비"], purchaseIntent:80, iconKey:0, color:"var(--primary)", iconBg:"#EEF4FF", description:"모바일 게임을 즐겨하는 10대. 고주사율 디스플레이와 쿨링 성능에 민감함.", techLevel:"중급", monthlyTechSpend:"5만원 미만", interests: ["모바일 e스포츠","웹툰","PC 하드웨어"], competitorPerception:"아이폰의 게이밍 성능을 부러워하나 안드로이드의 앱 생태계를 선호함.", marketingAcceptance: 90, futureValue: 75, purchaseHistory: ["S23 FE (2023)"], userLogs: ["게임 런처 통해 원신 2시간 플레이 (어젯밤)"], brandAttitude: 70 },
 ...
 { name:"서유리", age:27, gender:"여성", occupation:"일러스트레이터", device:"S24 Ultra", segments:["게이밍 성향군"], keywords:["S펜","드로잉","고해상도"], purchaseIntent:70, iconKey:0, color:"var(--primary)", iconBg:"#EEF4FF", description:"이동 중에도 아이디어를 스케치하는 20대 후반 프리랜서 작가.", techLevel:"전문가", monthlyTechSpend:"15만원 대", interests: ["웹툰","전시","일러스트"], competitorPerception:"아이패드를 메인으로 쓰지만, 스마트폰은 펜이 내장된 울트라가 편해서 씀.", marketingAcceptance: 80, futureValue: 82, purchaseHistory: ["S24 Ultra (2024)"], userLogs: ["Penup 앱으로 30분간 스케치 (오늘 오전)"], brandAttitude: 84 },
...
function DetailModal({ persona, onClose }: { persona: Persona; onClose: () => void }) {
 ...
 <div
 className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl border bg-card shadow-[var(--shadow-[var(--shadow-sm)])]"
 ...
 <span className="rounded-lg border border-[var(--primary-light-border)] bg-[var(--primary-light-bg)] px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-primary">
 LV.{Math.floor(persona.futureValue / 10)}
 </span>
 <span className={`rounded-lg border px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider ${urgencyCls}`}>
 {urgencyTone}
 </span>
 </div>
 ...
 <button
 onClick={onClose}
 className="flex h-10 w-10 items-center justify-center rounded-xl border border-[var(--border)] bg-card text-[var(--subtle-foreground)] shadow-[var(--shadow-[var(--shadow-sm)])] transition-all hover:bg-[var(--surface-hover)] hover:text-foreground active:scale-95"
 >
 <X size={20} />
 </button>
 </div>
 ...
 <button
 onClick={onClose}
 className="rounded-xl bg-primary px-10 py-3 text-[13px] font-bold text-white shadow-[var(--shadow-[var(--shadow-sm)])] transition-all hover:bg-primary-hover active:scale-95"
 >
 프로필 닫기
 </button>
 </div>
...
export const PersonaManagerPage: React.FC = () => {
 ...
 <button className="flex items-center gap-2.5 bg-primary text-white px-6 py-2.5 rounded-xl font-bold shadow-[var(--shadow-[var(--shadow-sm)])] hover:bg-primary-hover active:scale-95 transition-all text-[13px]">
 <Plus size={16} strokeWidth={2.5} /> 신규 등록
 </button>
 </div>
 </div>
 ...
 <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border shadow-[var(--shadow-[var(--shadow-sm)])]" style={{ backgroundColor: p.iconBg, borderColor: `${p.color}2e` }}>
 ...
 <span className={`rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase tracking-tight ${riskMeta.cls}`}>
 {riskMeta.label}
 </span>
 </header>
...
"{p.description}"
 ...
 className="inline-flex items-center gap-1.5 rounded-xl border border-[var(--border)] bg-[var(--panel-soft)] px-4 py-2.5 text-[12px] font-bold text-[var(--secondary-foreground)] transition-all hover:border-[var(--primary-light-border)] hover:bg-[var(--primary-light-bg)] hover:text-primary active:scale-95 shadow-[var(--shadow-[var(--shadow-sm)])]"
 >
 ...
 <th className="px-8 py-5 text-[10px] font-bold text-[var(--subtle-foreground)] uppercase tracking-[0.14em]">Persona Profile</th>
 <th className="px-8 py-5 text-[10px] font-bold text-[var(--subtle-foreground)] uppercase tracking-[0.14em] text-center">Brand / Intent</th>
 <th className="px-8 py-5 text-[10px] font-bold text-[var(--subtle-foreground)] uppercase tracking-[0.14em] text-center">Future Val</th>
 <th className="px-8 py-5 text-[10px] font-bold text-[var(--subtle-foreground)] uppercase tracking-[0.14em] text-center">Marketing</th>
 <th className="px-8 py-5 text-[10px] font-bold text-[var(--subtle-foreground)] uppercase tracking-[0.14em] text-center">Risk</th>
 <th className="px-8 py-5 text-right text-[10px] font-bold text-[var(--subtle-foreground)] uppercase tracking-[0.14em] pr-10">Analysis</th>
 </tr>
 </thead>
 <tbody>
 {paginated.map(p => (
 <tr key={p.id} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--surface-hover)] transition-colors group">
 <td className="px-8 py-5">
 <div className="flex items-center gap-4">
 <div className="w-11 h-11 rounded-xl border flex items-center justify-center shadow-[var(--shadow-[var(--shadow-sm)])] group-hover:border-primary/30 transition-colors bg-card" style={{ backgroundColor: p.iconBg, borderColor: p.color +"22" }}>
 ...
 <button onClick={() => setDetailTarget(p)} className="inline-flex items-center gap-1.5 rounded-xl border border-[var(--border)] bg-[var(--panel-soft)] px-4 py-2 text-[12px] font-bold text-[var(--secondary-foreground)] transition-all hover:border-[var(--primary-light-border)] hover:bg-[var(--primary-light-bg)] hover:text-primary shadow-[var(--shadow-[var(--shadow-sm)])]">
