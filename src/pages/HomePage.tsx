...
function WizardModal({ initialTemplate, onClose }: { initialTemplate?: SurveyType; onClose: () => void }) {
 ...
 <div className="w-10 h-10 rounded-xl bg-card flex items-center justify-center text-primary shadow-[var(--shadow-sm)] border border-[var(--primary-light-border)] shrink-0">
 <Brain size={20} />
 </div>
 ...
 ?"border-primary bg-primary-light-bg/30 shadow-[var(--shadow-md)] ring-1 ring-primary"
 :"hover:border-primary/30 hover:bg-card"
 }`}
 >
 <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-[var(--panel-soft)] border border-[var(--border)] shadow-[var(--shadow-sm)] transition-colors group-hover:bg-card">
 ...
 <div className="relative bg-card p-6 rounded-3xl border border-[var(--primary-light-border)] shadow-[var(--shadow-lg)]">
...
export function HomePage() {
 ...
 {/* Welcome Header */}
 <section className="app-card p-10 border-border/90 relative overflow-hidden group [box-shadow:var(--shadow-[var(--shadow-md)])]">
 ...
 <button
 onClick={() => openWizard()}
 className="inline-flex items-center gap-3 rounded-2xl bg-primary px-8 py-4 text-[15px] font-black text-white transition-all hover:bg-primary-hover shadow-[var(--shadow-lg)] active:scale-95"
 >
 <Plus size={20} strokeWidth={3} /> 새 프로젝트 설계 시작
 </button>
 <button className="inline-flex items-center gap-3 rounded-2xl border border-[var(--border)] bg-card px-8 py-4 text-[15px] font-black text-secondary-foreground transition-all hover:bg-[var(--surface-hover)] hover:border-[var(--border-hover)] shadow-[var(--shadow-sm)]">
 ...
 {/* Stats Summary (디자인 시스템 적용: 모노크롬 블루) */}
 <section className="grid grid-cols-4 gap-6">
 {[
 { label:"진행 중인 프로젝트", value:"4", sub:"개", icon: FileText, color:"text-primary", bg:"bg-[var(--primary-light-bg)]", border:"border-[var(--primary-light-border)]" },
 { label:"이번 달 완료", value:"12", sub:"건", icon: CheckCircle2, color:"text-primary", bg:"bg-[var(--panel-soft)]", border:"border-[var(--border)]", opacity:"opacity-80" },
 { label:"평균 응답 지수", value:"68%", sub:"평균", icon: BarChart2, color:"text-primary", bg:"bg-[var(--panel-soft)]", border:"border-[var(--border)]", opacity:"opacity-70" },
 { label:"등록 페르소나", value:"30", sub:"명", icon: Users, color:"text-primary", bg:"bg-[var(--panel-soft)]", border:"border-[var(--border)]", opacity:"opacity-60" },
 ].map(s => (
 <div key={s.label} className="app-card p-6 flex items-center gap-5 hover:border-[var(--border-hover)] hover:shadow-[var(--shadow-[var(--shadow-md)])] transition-all duration-300">
 <div className={`rounded-2xl p-3.5 ${s.bg} ${s.color} border ${s.border} shadow-[var(--shadow-sm)] ${s.opacity ||""}`}>
 <s.icon size={22} />
 </div>
 <div>
 <p className="text-[10px] font-bold text-[var(--subtle-foreground)] uppercase tracking-[0.14em] mb-1.5">{s.label}</p>
 <div className="flex items-baseline gap-1.5">
 <span className="text-[26px] font-bold text-foreground leading-none tracking-tight">{s.value}</span>
 <span className="text-[11px] font-semibold text-[var(--subtle-foreground)] uppercase tracking-tight">{s.sub}</span>
 </div>
 </div>
 </div>
 ))}
