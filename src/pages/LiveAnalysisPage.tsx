...
const WORD_SIZES: Record<number, { fontSize: number; color: string; weight: number }> = {
  ...
  5: { fontSize: 15, color: "#5B7DFF", weight: 600 },
...
const BAR_COLORS = ["#5B7DFF", "#6D7AF1", "#A5B4FC", "#BFD4FF", "#DCE4F3"];
...
const AVATAR_COLORS = ["#EEF4FF", "#F0FDF4", "#FFF7ED", "#FDF2F8", "#F0F9FF"];
const AVATAR_TEXT_COLORS = ["#5B7DFF", "#16A34A", "#EA580C", "#DB2777", "#0284C7"];
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 flex flex-col gap-4">
        ...
        <div className="app-stat-card px-6 py-4 flex items-center gap-6">
          <div className="flex-1">
            <div className="flex items-center justify-between mb-2">
              <span style={{ fontSize: 15, fontWeight: 700, color: "#1D1F3D" }}>
                ...
                <span style={{ color: "#9BA6B8", fontWeight: 500 }}>/{total.toLocaleString()} 응답 완료</span>
              </span>
              <span
                className="bg-[#EEF4FF] text-[#5B7DFF] px-2.5 py-0.5 rounded-full border border-[#BFD4FF]"
                style={{ fontSize: 12, fontWeight: 700 }}
              >
                {pct}%
              </span>
            </div>
            <div className="h-2.5 bg-[#E1E8F1] rounded-full overflow-hidden">
              <div
                className="h-full rounded-full transition-all duration-700"
                style={{
                  width: `${pct}%`,
                  background: "linear-gradient(90deg, #5B7DFF, #818CF8)",
                }}
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <div>
              <p style={{ fontSize: 11, color: "#9BA6B8" }}>예상 잔여 시간</p>
              <p style={{ fontSize: 13, fontWeight: 700, color: "#1D1F3D" }}>약 6분 40초</p>
            </div>
          </div>
          ...
          <div className="app-stat-card p-5 flex flex-col">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <span
                  className="bg-[#EEF4FF] text-[#5B7DFF] px-2 py-0.5 rounded-full"
                  style={{ fontSize: 10, fontWeight: 600 }}
                >
                  Live Analysis
                </span>
              </div>
              <button className="w-7 h-7 rounded-lg bg-[#EEF2FA] hover:bg-[#EEF4FF] flex items-center justify-center transition-colors">
                <RefreshCw size={12} className="text-[#7C8397]" />
              </button>
            </div>
                ...
                {feed.slice(0, 4).map((item, idx) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p
                        className="truncate"
                        style={{ fontSize: 12, color: "#1D1F3D", lineHeight: 1.4 }}
                      >
                        <span style={{ fontWeight: 600 }}>{item.time}</span>
                        {" · "}
                        <span style={{ color: "#5B7DFF", fontWeight: 600 }}>{item.user}</span>
                        {" · "}
                        <span style={{ color: "#3C4556" }}>{item.content}</span>
                      </p>
                    </div>
                  </div>
                ))}
